...
class KinematicObservation(ObservationType):
    ...
    FEATURES: List[str] = ["presence", "x", "y", "vx", "vy", "a", "j", "ttc_f", "ttc_r"]  # obs1
    # FEATURES: List[str] = ["presence", "x", "y", "vx", "vy", "a", "j", "ttc_f", "ttc_r",
    #                        "ttc", "risk_value", "risk_gradient"]  # obs2
    ...
    def space(self) -> spaces.Space:
        return spaces.Box(
            # shape=(self.vehicles_count * self.env.config["controlled_vehicles"], len(self.features)),
    ...
    def normalize_obs(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self.features_range:
            ...
            # }
            self.features_range = {
                "x": [-30.0 * Vehicle.MAX_SPEED, 30.0 * Vehicle.MAX_SPEED],
                "y": [-50, 50],
                "vx": [-2 * Vehicle.MAX_SPEED, 2 * Vehicle.MAX_SPEED],
                "vy": [-2 * Vehicle.MAX_SPEED, 2 * Vehicle.MAX_SPEED],
                "a": [-50, 50],
                ...
                "ttc_lon": [-1e3, 1e3],
                "ttc": [-1e3, 1e3],
                "risk_value": [-self.observer_vehicle.max_risk, self.observer_vehicle.max_risk],
                "risk_gradient": [-2 * self.observer_vehicle.max_risk, 2 * self.observer_vehicle.max_risk]
            }
    ...
    def observe(self) -> np.ndarray:
        ...
        # Add nearby traffic
        # Kong Added: observe the closest 4 cars
...
class MultiAgentObservation(ObservationType):
    ...
    def space(self) -> spaces.Space:
        # !!!!!!!!!!!9 <-- in class KinematicsObservation  self.vehicles_count = 9
        return spaces.Box(
            shape=(int(self.env.config["controlled_vehicles"] * 5), len(self.FEATURES)),
            low=-np.inf,
            high=np.inf,
            dtype=np.float32,
        )
    ...
    def observe(self) -> tuple:
        return tuple(np.vstack(tuple(obs_type.observe() for obs_type in self.agents_observation_types)))
