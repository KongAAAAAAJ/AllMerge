...
class AbstractEnv(gym.Env):
    ...
    speed. The action_num space is fixed, but the observation space and reward function must be defined in the
    ...
    def default_config(cls) -> dict:
        ...
        return {
            ...
            "w_game split weights": [30, 0, 0, 0, 5, 8],  # 效率、连通性、x error、y error, 安全性, 横向距离
            "w_game merge weights": [50, 1e3, 1, 100, 5, 8],  # 效率、连通性、x error、y error, 安全性， 横向距离
    ...
    def define_spaces(self) -> None:
        """
        Set the types and spaces of observation and action_num from config.
        """
        self.observation_type = observation_factory(self, self.config["observation"])
        self.action_type = action_factory(self, self.config["action_num"])
        self.observation_space = self.observation_type.space()
        self.action_space = self.action_type.space()

    def _reward(self, action: Action) -> float:
        """
        Return the reward associated with performing a given action_num and ending up in the current state.

        :param action: the last action_num performed
        :return: the reward
        """
    ...
    def reset(
        ...
        *,
        ...
        self.define_spaces()  # First, to set the controlled vehicle class depending on action_num space
        longitudinal = None
        if "Controller" in self.config:
            from highway_env.vehicle.longitudinal import LongitudinalControl
            longitudinal = LongitudinalControl(
                self.config["Controller"], 1 / self.config["simulation_frequency"])
        self.time = self.steps = 0
        self.done = False
        self._reset()
        if longitudinal is not None:
            self.road.longitudinal_control = longitudinal
        ...
        if self.config["Decision maker"] == "Game":
            """Game Decision"""
            self.maker = COALITION_GAME_MAKER(env=self)
        else:
            """Rule Decision"""
            self.maker = RULE_MAKER(env=self)
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, bool, dict]:
        """
        Perform an action_num and step the environment dynamics.

        The action_num is executed by the ego-vehicle, and all other vehicles on the road performs their default behaviour
        for several simulation timesteps until the next decision making step.

        :param action: the action_num performed by the ego-vehicle
        ...
        self._simulation_trace = [self._vehicle_plot_sample()]
        ...
        info["simulation_trace"] = self._simulation_trace
        # Capture terminal state before DummyVecEnv automatically resets us.
        info["episode_state"] = {
            "time": float(self.time),
            "terminated": bool(terminated),
            "truncated": bool(truncated),
            "crashed": [bool(v.crashed) for v in self.controlled_vehicles],
            "lane_index": [tuple(v.lane_index) for v in self.controlled_vehicles],
            "on_road": [bool(v.on_road) for v in self.controlled_vehicles],
            "offroad_terminal": bool(self.config.get("offroad_terminal", False)),
        }
        if self.render_mode == "human":
            self.render()
    ...
    def _vehicle_plot_sample(self) -> dict:
        """Copy actual vehicle state; keep native solver objects out of info."""
        sample = {
            "time": float(self.steps / self.config["simulation_frequency"]),
            "position": [[float(x) for x in v.position] for v in self.controlled_vehicles],
            "front_wheel_angle_deg": [
                float(v.front_wheel_angle_deg) if hasattr(v, "trucksim_model")
                else float(np.rad2deg(v.action["steering"]))
                for v in self.controlled_vehicles
            ],
            "speed": [float(v.speed) for v in self.controlled_vehicles],
            "acceleration": [
                float(v.measured_acceleration[0]) if hasattr(v, "measured_acceleration")
                else float(v.action["acceleration"])
                for v in self.controlled_vehicles
            ],
        }

        if any(hasattr(v, "trucksim_model") for v in self.controlled_vehicles):
            sample["trucksim_outputs"] = [v.export_array.copy() for v in self.controlled_vehicles]
            sample["solver_time"] = [float(v.trucksim_model.current_time) for v in self.controlled_vehicles]
        return sample

    def _simulate(self, action: Optional[Action] = None) -> None:
        """Perform several steps of simulation with constant action_num."""
        frames = int(
            self.config["simulation_frequency"] // self.config["policy_frequency"]
        )
        for frame in range(frames):
            # Forward action_num to the vehicle
            # if (
            #     action is not None
            ...
            self.steps += 1
            self._simulation_trace.append(self._vehicle_plot_sample())

            utils.data_record_each_step(self, action)
    ...
    def _automatic_rendering(self) -> None:
        """
        Automatically render the intermediate frames while an action_num is still ongoing.
