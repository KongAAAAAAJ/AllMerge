...
class CounterfactualScoringMixin:
    def _score_target_group(
        ...
        *,
        ...
        dense_local, _ = _dense_local_trajectories(
            joint,
            self.config,
        )
        dense_world = np.empty_like(
            dense_local,
            dtype=np.float64,
        )
        ...
        progress_score = np.zeros(
            group_size,
            dtype=np.float64,
        )
        gap_penalty = np.zeros(
            group_size,
            dtype=np.float64,
        )
        ttc_penalty = np.zeros(
            group_size,
            dtype=np.float64,
        )
        road_penalty = np.zeros(
            group_size,
            dtype=np.float64,
        )
        ...
        minimum_ttc = np.full(
            group_size,
            self.config.no_risk_ttc_s,
            dtype=np.float64,
        )

        tracking_dimensions = tracking_aware_dimensions(
            self.config
        )
        physical_dimensions = (
        ...
        for group in range(group_size):
            ...
            road_margin = road_margin_series(
                world,
                road,
                self.config,
                tracking_aware=True,
            )
            minimum_road_margin[group] = float(
                np.min(road_margin)
            )
            out_of_drivable[group] = bool(
                np.any(road_margin[1:] < 0.0)
            )
            road_risk = soft_threshold_risk(
                road_margin,
                warning_threshold=(
                    self.config.road_margin_warning_m
                ),
                softness=(
                    self.config.road_margin_softness_m
                ),
            )
            road_penalty[group] = aggregate_temporal_risk(
                road_risk,
                max_weight=(
                    self.config.temporal_max_weight
                ),
                mean_weight=(
                    self.config.temporal_mean_weight
                ),
            )
            ...
            for actor, predictions in (
                background_by_actor.items()
            ):
                ...
                for (
                    ...
                    branch_ttc_risks.append(
                        soft_threshold_risk(
                            ttc,
                            warning_threshold=(
                                self.config.ttc_warning_s
                            ),
                            softness=(
                                self.config.ttc_softness_s
                            ),
                        )
                    )
                ...
                if branch_gap_risks:
                    interaction_gap_risks.append(
                        np.max(
                            np.stack(branch_gap_risks),
                            axis=0,
                        )
                    )
                    interaction_ttc_risks.append(
                        np.max(
                            np.stack(branch_ttc_risks),
                            axis=0,
                        )
                    )
            ...
            gap_penalty[group] = aggregate_temporal_risk(
                np.max(
                    np.stack(interaction_gap_risks),
                    axis=0,
                ),
            ...
            ttc_penalty[group] = aggregate_temporal_risk(
                np.max(
                    np.stack(interaction_ttc_risks),
                    axis=0,
                ),
                max_weight=self.config.temporal_max_weight,
                mean_weight=self.config.temporal_mean_weight,
        ...
        return {
            ...
            "components": {
                "progress_score": (
                    progress_score.astype(np.float32)
                ),
                "gap_penalty": (
                    gap_penalty.astype(np.float32)
                ),
                "ttc_penalty": (
                    ttc_penalty.astype(np.float32)
                ),
                "road_penalty": (
                    road_penalty.astype(np.float32)
                ),
                "comfort_penalty": (
                    comfort_penalty.astype(np.float32)
                ),
                "minimum_background_gap_m": (
                    minimum_background_gap.astype(np.float32)
                ),
                "minimum_teammate_gap_m": (
                    minimum_teammate_gap.astype(np.float32)
                ),
                "minimum_road_margin_m": (
                    minimum_road_margin.astype(np.float32)
                ),
                "minimum_ttc_s": (
                    minimum_ttc.astype(np.float32)
                ),
            },
        }
