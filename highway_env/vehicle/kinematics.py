...
class Vehicle(RoadObject):
        ...
        if speed is None:
            if lane.speed_limit is not None:
                speed = road.np_random.uniform(
                    ...
                    # 0.4 * lane.speed_limit, 0.5 * lane.speed_limit
                )
            else:
                speed = road.np_random.uniform(
    ...
    def act(self, action: Union[dict, str] = None) -> None:
        """
        Store an action_num to be repeated.

        :param action: the input action_num
        """
        if action:
            self.action = action
    ...
    def predict_trajectory(
        ...
        for action in actions:
            v.act(action)  # Low-level control action_num
