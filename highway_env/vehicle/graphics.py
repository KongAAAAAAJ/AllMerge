...
from highway_env.vehicle.behavior import IDMVehicle, LinearVehicle, AggressiveIDMVehicle
from highway_env.vehicle.controller import MDPVehicle, FOLLOWVehicle
...
class VehicleGraphics(object):
    ...
    def display_trajectory(
        cls, vehicle: FOLLOWVehicle, surface: "WorldSurface", offscreen: bool = False
    ) -> None:
        """
        Display the planned future trajectory of a vehicle on a pygame surface.
        ...
        if path is None:
            return
        vehicle_future_lists = []
        for x_position, y_position, ref_steering_angle, ref_heading in zip(path["x"], path["y"], path["steering"], path["heading"]):
            copy_vehicle = copy.deepcopy(vehicle)
            copy_vehicle.position = [x_position, y_position]
            copy_vehicle.heading = ref_heading
            copy_vehicle.action["steering"] = ref_steering_angle
            vehicle_future_lists.append(copy_vehicle)
        for vehicle in vehicle_future_lists:
    ...
    def get_color(cls, vehicle: Vehicle, transparent: bool = False) -> Tuple[int]:
        ...
        if getattr(vehicle, "color", None):
            ...
            color = cls.YELLOW
        elif isinstance(vehicle, AggressiveIDMVehicle):
            color = cls.YELLOW
        elif isinstance(vehicle, IDMVehicle):
            color = cls.BLUE
        elif isinstance(vehicle, MDPVehicle):
            color = cls.EGO_COLOR
        elif isinstance(vehicle, FOLLOWVehicle):
            color = cls.EGO_COLOR
        if transparent:
