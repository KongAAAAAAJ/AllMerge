...
class FOLLOWVehicle(ControlledVehicle):
    ...
    def __init__(
        ...
        width: float = 2,
        timer: float = None,
        group_action: int = 3,  # 初始为merge状态
    ) -> None:
        """
        Initializes an FOLLOWERVehicle
    ...
    def polynomial_planning(self, index_group, car_id, controller_vehicles, lane_index) -> tuple:
        """
        Using polynomial planner
        """
        ...
        poly_planner = PolyPlanner(
            controlled_vehicle=vehicle, vehicle_type=vehicle_type, road=self.road, target_lane_index=lane_index,
            lead_vehicle=lead_v, follow_index=follow_idx, group=group
        )
        ref_headings, ref_speeds, ref_steering_angles, x_positions, y_positions, LTR_values = poly_planner.planning()
        path = {
            "x": x_positions,
            "y": y_positions,
        ...
        LTR = max(LTR_values) if LTR_values is not None else None
    ...
    def act(
        ...
        :param lane_index: the target lane index
        :param acceleration: the continuous acceleration
        :param planner_flag: using planner or not
        """
        self.follow_road()
        if lateral_action is None:
            return
            ...
            if planner is not None and planner["state"] is True:
                if planner["type"] == "Polynomial":
                    ref_steering_angles, path = self.polynomial_planning(
                        index_group=index_group, car_id=car_id, controller_vehicles=controlled_vehicles,
                        lane_index=lane_index
                    )
                    ref_y_postion = path["y"][10]
                    ref_heading = path["heading"][10]
                    action = {
                        "steering": self.steering_pid_control(ref_y_postion, ref_heading),
                        "acceleration": acceleration
                    }
                else:
                    path = None
                ...
                action = {
                    "steering": self.steering_control(self.target_lane_index, heading_command),
                    "acceleration": acceleration,
                }
