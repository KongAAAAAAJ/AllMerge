...
def check_f1_f2_contract(
    *,
    ...
    if not np.array_equal(
        ...
        errors.append(
            "F.1 geometry_semantic target_mode "
            f"{f1_mode.tolist()} != "
            f"F.2 target_mode {target_mode.tolist()}"
        )
...
def main():
    ...
    while len(
        ...
        try:
            ...
            # Intentionally step the raw VecEnv returned by env_reset rather
            # than VecVideoRecorder. This avoids producing hundreds of pilot
            # video frames and does not alter the environment/planner logic.
            runner_env = env
